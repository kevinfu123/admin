      ...
      methods:{
         ...
         //添加分类确认时间
          ...
          handleSuccess1(response, file, fileList) {
            this.nologyform.brand_logo = response.file
          },
          ...
          serveopen(formName) {
              this.$refs[formName].validate((valid) => {
                  if (valid) {
                      //拆分id
                      var ids = []
                      if(this.nologyform.cate_id.length >1){
                        this.nologyform.cate_id.map((item) => {
                          ids.push(Number(item.c_id));
                        }); 
                        this.nologyform.cate_id = ids.join(',');
                      }else{
                        this.nologyform.cate_id = this.nologyform.cate_id[0].c_id;
                      }
