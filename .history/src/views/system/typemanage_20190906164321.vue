        ...
        getList() {
            typeList(this.listQuery).then((result) => {
              ...
              for (let i = 0; i < len; i++) {
                  ...
                  this.$set(this.openicon[i],'folded',false);
                  this.$set(this.openicon[i],'id',this.dataList[i].id);
                  this.$set(this.openicon[i],'value',this.dataList[i].id);
                  this.$set(this.openicon[i],'label',this.dataList[i].cate_name);
                  //设置第二层级的状态
                  for (let j in this.dataList[i].children) {
                      this.isOpenItem[i].child.push(true)

                      this.$set(this.openicon[i].children[j],'value',this.dataList[i].children[j].id);
                      this.$set(this.openicon[i].children[j],'label',this.dataList[i].children[j].cate_name);
                      for(let k in this.dataList[i].children[j].children) {
                        this.$set(this.openicon[i].children[j].children[k],'value',this.dataList[i].children[j].children[k].id);
                        this.$set(this.openicon[i].children[j].children[k],'label',this.dataList[i].children[j].children[k].cate_name);
                      }
                  }
              }
            }).catch((err) => {
