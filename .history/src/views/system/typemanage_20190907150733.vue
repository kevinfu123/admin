        ...
        <el-dialog :title="dialogTitle" width="700px"  @close="handleClose" class="demo-ruleForm" :visible.sync="dialogFormVisible">
            <el-form :model="nologyform" status-icon :rules="rules" label-position="right" label-width="155px" ref="nologyform">
                ...
                <el-form-item style="margin-bottom: 16px;" prop="index_nav_sort" label="排序：">
                    <el-input style="width:60%;"  size="mini" v-model.number="nologyform.index_nav_sort"></el-input>
                </el-form-item>
                ...
                <el-form-item  label="排序：">
                    <el-input style="width:60%;"  size="mini" v-model.number="nologyform.floor_nav_sort"></el-input>
                </el-form-item>
                ...
                <el-form-item  label="排序：">
                    <el-input style="width:60%;"  size="mini" v-model.number="nologyform.index_needs_sort"></el-input>
                </el-form-item>
                <el-form-item  label="特殊技术费率：">
                    <el-input :disabled="tech_ratedisabled" style="width:30%" size="mini" v-model.number="nologyform.tech_rate"></el-input><span style="color:red;margin-left:5px;">注意:这个限制于三级类目</span>
                </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button size="mini" type="primary" @click="typemanageopen('nologyform')">确 定</el-button>
            </div>
        </el-dialog>
    </el-main>
   </el-container>
...
<script>
    import {edittype, typeList, addtype, editstatus, catecut} from '@/api/system/typemanage';
    ...
    export default {
      ...
      data() {
        return {
            ...
            rules: {
              ...
              index_nav_sort:[{ required: true, message: '首页导航栏不能为空'},{ type: 'number', message: '首页导航栏排序为数字'}],
            },
      ...
      methods:{
        ...
        handleAddClick() {
            ...
            this.nologyform.cate_description = '';
            this.nologyform.cate_keywords = '';
        ...
        //添加、编辑一级分类
        typemanageopen(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    if(this.nologyform.pid.length<2){
                       this.nologyform.pid = this.nologyform.pid[0]
                    }else{
                       this.nologyform.pid = this.nologyform.pid[1]
                    }
                    if(this.nologyform.id){//编辑
                        
                        
                    }else{//新增
                       addtype(this.nologyform).then((result) => {
                          this.$message({
                              type: 'success',
                              message: '新增成功!'
                          });
